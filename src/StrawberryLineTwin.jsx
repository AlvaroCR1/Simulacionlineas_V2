// ─── StrawberryLineTwin.jsx  v4.0 ────────────────────────────────────────────
// Tres estados visuales de tarrina:
//   VERDE  (0x00cc66) — OK en M1/M2/M4
//   ROJO   (0xee2211) — Rechazada (flag=8) en M1 o M2
//   NARANJA(0xff2200) — Rechazo definitivo en GLMI (trampilla)
//
// Comportamientos físicos:
//   M1 REJ → desvío en Z (-1.2u) hacia embudo · vuelve al eje central antes de M2
//   M2 REJ → baja en Y (-0.8u) hacia módulo circular · sube al llegar a M4
//   GLMI REJ → cae en Y (trampilla) y desaparece
//
// Play/Pause lo controla el TimelineEngine vía onTick.
// Sidebar: solo estado, controles físicos y chatbot. Sin START/STOP ni slider.
...
// Velocidades base calculadas de datos reales (se escalan por engine.speed)
//   M1→M2: 6.57u / 13.8s = 0.476 u/s
//   M2→M4: 18.83u / 456.6s = 0.041 u/s  (incluye tiempo en módulo circular)
// Velocidades de referencia (ya no se usan directamente — velocidad visual es fija)
const V_M1M2 = 1.5; // u/s visual M1→M2
const V_M2M4 = 0.8; // u/s visual M2→M4
const V_POST = 1.5; // u/s visual tras GLMI

// Color Three.js por tipo de pesada
const COL = {
  ok: 0x00cc66,
  reject: 0xee2211,
  sensor: 0xff8800,
...
function makeInitialState() {
  return {
    ...
    glmiCountTimestamps: [],
    glmiGroup: null, // referencia al grupo M4 para la trampilla
    running: false,
    trays: [],
    trayMap: {},
    ctrlStates: {},
    ...
    mouse: null,
    _engineSpeed: 10,
    _spawnQueue: [], // cola de spawns pendientes si THREE aún no está listo
    orbit: {
      theta: 0.05,
      phi: 1.05,
      radius: 22,
...
// ─── _doSpawn — lógica de spawn extraída para reutilizarla desde la cola ────
function _doSpawn(THREE, st, tray) {
  console.log("SPAWN", tray.id, st.trays.length);
  ...
  // Velocidad VISUAL fija — sincronización real la hacen onReachM2/onReachM4
  const speedM1M2 = V_M1M2;
...
// THREE se pasa como parámetro ya que es un global cargado dinámicamente
...
export default function StrawberryLineTwin({ engine }) {
  ...
  // ── Three.js init ──────────────────────────────────────────────────────────
  useEffect(() => {
    stRef.current._engine = engine;
    stRef.current._engineConnected = false;
  ...
  useEffect(() => {
    const st = stRef.current;
    if (!engine) {
      setUi((p) => ({ ...p, engineLoaded: false }));
      return;
    }
    setUi((p) => ({ ...p, engineLoaded: true }));

    // Sincronizar running con play/pause del engine
    engine.onTick = (state) => {
    ...
    // ── SPAWN: tarrina nace en M1 ────────────────────────────────────────────
    engine.onSpawn = (tray) => {
      console.log("PUSHED", tray.id);
      const THREE = window.THREE;
      const st = stRef.current;
      if (THREE && st.scene) {
        _doSpawn(THREE, st, tray);
      }
    };
    console.log("onSpawn asignado:", !!engine.onSpawn);

    // ── LLEGA A M2: recolorar y cambiar fase ─────────────────────────────────
    engine.onReachM2 = (tray) => {
      const obj = st.trayMap[tray.id];
      if (!obj) return;

      // Recolorar cuerpo según resultado en M2
      ...
      // Si rechazada en M2 → bajar al módulo circular
      obj.phase = "M2M4";
      obj.speedX = V_M2M4;
      obj.destX = SEG_CX.M4 + 0.8;
      obj.segId = "M2";
      obj.targetZ = 0; // vuelve al eje central
      // Bajar ligeramente si rechazada (módulo circular)
      // Si rechazada en M2 → baja 0.6u (módulo circular por debajo de la cinta)
      // Si OK → mantiene altura normal
      obj.targetY =
    ...
    // ── LLEGA A M4 (GLMI): recolorar y fase final ────────────────────────────
    engine.onReachM4 = (tray) => {
      ...
      if (isGlmiRej) {
        // Trampilla: colorear rojo y activar animación de apertura
        const body = obj.mesh.children[0];
        if (body?.material) {
          body.material.color.setHex(COL.glmi_rej);
          body.material.needsUpdate = true;
        }
        // Abrir trampilla en el modelo 3D
        ...
        obj.isFalling = true;
        obj.speedX = 0;
        obj.speedY = -2.5; // u/s caída libre
        obj.destX = SEG_CX.M4 + 2; // ya no importa X
      } else {
        ...
        // Vuelve a subir al nivel de cinta si estaba bajada
        obj.targetY = BELT_TOP + 0.065;
        obj.phase = "M4out";
        obj.speedX = V_POST;
        obj.destX = SEG_CX.M5 + 5; // sale de escena
        obj.segId = "M4";
      }
    };
  ...
  function initThree(st, container) {
    ...
    function onMM(e) {
      ...
      if (o.isDragging) {
        const dx = e.clientX - o.lastX,
          dy = e.clientY - o.lastY;
        if (
          Math.abs(e.clientX - o.dragStartX) > 3 ||
          Math.abs(e.clientY - o.dragStartY) > 3
        )
          o.moved = true;
        o.theta -= dx * 0.008;
      ...
      if (o.isRightDragging) {
        ...
        const fx = cp.x - o.targetX,
          fy = cp.y - o.targetY,
          fz = cp.z - o.targetZ;
        const fl = Math.sqrt(fx * fx + fy * fy + fz * fz);
        const rx = (fy / fl) * 0 - (fz / fl) * 1,
          rz = (fx / fl) * 1 - (fy / fl) * 0;
        const rl = Math.sqrt(rx * rx + rz * rz),
          pan = o.radius * 0.0012;
        o.targetX = Math.max(
          -43,
          Math.min(37, o.targetX + (rx / rl) * dx * pan)
        );
        o.targetZ = Math.max(
          -15,
          Math.min(15, o.targetZ + (rz / rl) * dx * pan)
        );
    ...
    function onW(e) {
      st.orbit.radius = Math.max(
        4,
        Math.min(55, st.orbit.radius * (1 + e.deltaY * 0.001))
      );
      updateCamera(THREE, st);
    }
    ...
    function animate(time) {
      // Conectar engine cuando esté disponible
      st._connectEngine = (eng) => {
        eng.onSpawn = (tray) => {
          console.log("PUSHED", tray.id);
          _doSpawn(THREE, st, tray);
        };
      ...
      if (engine) st._connectEngine(engine);
      rafRef.current = requestAnimationFrame(animate);
      if (!st.lastTime) st.lastTime = time;
      const delta = Math.min((time - st.lastTime) / 1000, 0.05);
      st.lastTime = time;

      // Conectar engine si aún no está conectado
      const eng = stRef.current._engine;
      if (eng && !st._engineConnected) {
        st._engineConnected = true;
        eng.onSpawn = (tray) => {
          console.log("PUSHED", tray.id);
          _doSpawn(THREE, st, tray);
        };
      ...
      console.log("LOOP", !!window.THREE, !!st.scene, st._spawnQueue.length);
      // Vaciar cola de spawns pendientes (puede haber si Three.js cargó tarde)
      const THREE_G = window.THREE;
      if (THREE_G && st.scene && st._spawnQueue.length > 0) {
        console.log("QUEUE", st._spawnQueue.length, !!THREE_G, !!st.scene);
        const batch = st._spawnQueue.splice(0, 20);
        batch.forEach((tray) => _doSpawn(THREE_G, st, tray));
      }
      ...
      // ── Mover tarrinas ──────────────────────────────────────────────────────
      const spd = st._engineSpeed || 10;
      const toRemove = [];

      st.trays.forEach((t) => {
        if (!st.running && !t.isFalling) return;

        const mesh = t.mesh;
        const pos = mesh.position;

        // Interpolación suave de Z (desvío lateral embudo M1)
        if (t.targetZ !== undefined && Math.abs(pos.z - t.targetZ) > 0.01)
          pos.z += (t.targetZ - pos.z) * Math.min(delta * 4, 1);

        // Interpolación suave de Y (módulo circular M2 / trampilla GLMI)
        if (
          t.targetY !== undefined &&
          !t.isFalling &&
          Math.abs(pos.y - t.targetY) > 0.005
        )
          pos.y += (t.targetY - pos.y) * Math.min(delta * 3, 1);

        if (t.isFalling) {
          // Caída libre (trampilla GLMI)
          ...
          // Avance horizontal — velocidad visual FIJA, no escala con engineSpeed
          // La sincronización temporal la hacen los callbacks del engine
        ...
        //         // Eliminar en fase final o si cruzan el borde derecho de la escena
        if (!t.isFalling && t.phase === "M4out" && pos.x > t.destX) {
          toRemove.push(t);
          return;
        }
        // Seguridad: eliminar cualquier tarrina que salga del borde derecho
      ...
      st.balizaObjects.forEach((b) => {
        const isStop =
          b.dome.userData.stopUntil && nowMs < b.dome.userData.stopUntil;
        if (!st.running) {
          ...
          if (b.light) {
            b.light.color.setHex(0xff0000);
            b.light.intensity = 1.5;
          }
        } else {
          const p = 1.8 + 1.2 * Math.sin(st.elapsed * 6);
          b.dome.material.color.setHex(0x00ff00);
          b.dome.material.emissive.setHex(0x00ff00);
          b.dome.material.emissiveIntensity = -1.0;
          b.dome.material.opacity = 1.0;
          if (b.light) {
            b.light.color.setHex(0x39ff14);
            b.light.intensity = p * 0.8;
          }
        }
      });
      ...
      const statuses = {};
      SEGMENTS.forEach((s) => {
        statuses[s.id] = !st.running
          ? "stopped"
          : st.pausedSegments.has(s.id)
          ? "paused"
          : "running";
      });
      setUi((prev) => ({
  ...
  function handleClick(e, THREE, st) {
    ...
    for (const hit of hits) {
      let obj = hit.object;
      while (obj) {
        if (obj.userData?.ctrlId) {
          toggleCtrl(obj.userData.ctrlId, st);
          return;
        }
        obj = obj.parent;
      }
  ...
  // buildGround, buildScene, buildBelt, buildModular, buildBaliza
  // (idénticos a la versión anterior — omitidos por brevedad pero presentes)
  function buildGround(THREE, scene, st) {
    ...
    for (let i = 0; i < 300; i++) {
      const x = Math.random() * 1024,
        y = Math.random() * 1024,
        r = Math.random() * 12 + 2,
        gv = Math.floor(Math.random() * 15);
      ctx.fillStyle = `rgba(${180 + gv},${176 + gv},${168 + gv},0.06)`;
      ctx.beginPath();
      ctx.arc(x, y, r, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.strokeStyle = "rgba(30,70,110,0.25)";
    ctx.lineWidth = 2;
    [0, 256, 512, 768, 1024].forEach((x) => {
      ctx.beginPath();
      ctx.moveTo(x, 0);
      ctx.lineTo(x, 1024);
      ctx.stroke();
    });
    [0, 256, 512, 768, 1024].forEach((y) => {
      ctx.beginPath();
      ctx.moveTo(0, y);
      ctx.lineTo(1024, y);
      ctx.stroke();
    ...
    wctx.fillStyle = "#f0ede8";
    wctx.fillRect(0, 0, 512, 256);
    wctx.strokeStyle = "rgba(200,196,190,0.6)";
    wctx.lineWidth = 1.5;
    for (let rw = 0; rw < 8; rw++) {
      const off = (rw % 2) * 64;
      for (let c = -1; c < 5; c++)
        wctx.strokeRect(c * 128 + off, rw * 32, 128, 32);
    }
    ...
    rctx.fillStyle = "#9aa4aa";
    rctx.fillRect(0, 0, 512, 128);
    rctx.strokeStyle = "rgba(70,80,88,0.4)";
    rctx.lineWidth = 2;
    ...
    rft.wrapS = rft.wrapT = THREE.RepeatWrapping;
    rft.repeat.set(16, 4);
    ...
    roof.position.set(-3, GROUND_Y + WH, 0);
    scene.add(roof);
    ...
    [-35, -19, -3, 13, 29].forEach((bx) => {
      ...
      b.position.set(bx - 3, GROUND_Y + WH - 0.2, 0);
      b.castShadow = true;
      scene.add(b);
      const tf = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.08, ND), bm);
      tf.position.set(bx - 3, GROUND_Y + WH - 0.02, 0);
      scene.add(tf);
      const bf = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.08, ND), bm);
      bf.position.set(bx - 3, GROUND_Y + WH - 0.4, 0);
      scene.add(bf);
    });
    const ridge = new THREE.Mesh(new THREE.BoxGeometry(NW, 0.22, 0.22), bm);
    ridge.position.set(-3, GROUND_Y + WH - 0.11, 0);
    scene.add(ridge);
    const fbm = new THREE.MeshStandardMaterial({
      color: 0xd0d8dc,
      roughness: 0.4,
      metalness: 0.6,
    });
    ...
    [-28, -12, 4, 20].forEach((lx) => {
      [-4, 4].forEach((lz) => {
        ...
        fix.position.set(lx - 3, GROUND_Y + WH - 0.52, lz);
        scene.add(fix);
        const pan = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.02, 0.28), pgm);
        pan.position.set(lx - 3, GROUND_Y + WH - 0.58, lz);
        scene.add(pan);
        const pl = new THREE.PointLight(0xfff5e0, 1.1, 14);
        pl.position.set(lx - 3, GROUND_Y + WH - 0.65, lz);
        scene.add(pl);
      });
    });
  }
  function buildScene(THREE, scene, st) {
    ...
    // ── Elementos físicos adicionales ─────────────────────────────────────
    // Módulo circular bajo CM
    buildModularCircular(THREE, scene, -14.32);
    // Nota: la trampilla ya está dentro de buildGLMI y se activa vía
    // st.glmiGroup.userData.openTrapDoor()
    const { controlObjects, initialCtrlStates } = buildAllControls(
      THREE,
      scene
    );
  ...
  function buildBelt(THREE, group, seg, st) {
    ...
    [-1, 1].forEach((s) => {
      ...
      c.position.set(0, 0.11, s * (seg.d / 2 + 0.02));
      group.add(c);
    });
    [-1, 1].forEach((s) => {
      const r = new THREE.Mesh(
        new THREE.CylinderGeometry(0.09, 0.09, seg.d, 14),
        rm
      );
      r.rotation.x = Math.PI / 2;
      r.position.set((s * seg.w) / 2, 0.09, 0);
      group.add(r);
    });
  ...
  function buildModular(THREE, group, seg, st) {
    const imgEl = st.loadedTextures["cintaMod"] || null;
    const lg = new THREE.MeshStandardMaterial({
      color: 0xb0bcc4,
      roughness: 0.35,
      metalness: 0.55,
    });
    ...
    [-2.1, -0.5, 1.1].forEach((xo) => {
      [-0.44, 0.44].forEach((zo) => {
        const l = new THREE.Mesh(
          new THREE.CylinderGeometry(0.038, 0.038, 0.98, 8),
          lg
        );
        l.position.set(xo, -0.72, zo);
        l.castShadow = true;
        group.add(l);
        const f = new THREE.Mesh(
          new THREE.CylinderGeometry(0.055, 0.062, 0.04, 8),
          lg
        );
        f.position.set(xo, -1.2, zo);
        group.add(f);
      });
    });
    const surf = new THREE.Mesh(new THREE.BoxGeometry(seg.w, 0.07, seg.d), lg);
    surf.position.y = -0.26;
    surf.castShadow = true;
    group.add(surf);
    [-1, 1].forEach((s) => {
      const ch = new THREE.Mesh(
        new THREE.BoxGeometry(seg.w, 0.08, 0.045),
        new THREE.MeshStandardMaterial({
          color: 0x9aaabb,
          roughness: 0.5,
          metalness: 0.5,
      ...
      ch.position.set(0, 0.08, s * (seg.d / 2 + 0.022));
      group.add(ch);
    });
    ...
    group.add(belt);
    if (imgEl) {
      const pt = makePhotoTex(THREE, imgEl, (ctx) => {
        ctx.fillStyle = "rgba(10,18,30,0.35)";
        ctx.fillRect(0, 0, 512, 512);
      });
      const pf = new THREE.Mesh(
        new THREE.PlaneGeometry(seg.w * 0.92, 0.62),
        new THREE.MeshStandardMaterial({
          map: pt,
          roughness: 0.5,
          metalness: 0.1,
      ...
      pf.position.set(0, -0.08, seg.d / 2 + 0.005);
      group.add(pf);
    }
    [-2.0, -0.65, 0.7, 2.05].forEach((xo) => {
      ...
      mctx.fillStyle = "#001428";
      mctx.fillRect(0, 0, 128, 96);
      mctx.fillStyle = "#1a4a8a";
      mctx.font = "bold 13px Arial";
      ...
      mS.position.set(xo, 0.62, 0.037);
      group.add(mS);
      const md = new THREE.Mesh(
        new THREE.CylinderGeometry(0.05, 0.05, 0.015, 10),
        lg
      );
      md.position.set(xo, 0.295, 0);
      group.add(md);
    });
    [-2.1, -1.05, 0, 1.05, 2.1].forEach((xo) => {
      [-0.72, 0.72].forEach((zo) => {
        ...
        back.rotation.x = zo > 0 ? -Math.PI / 8 : Math.PI / 8;
        group.add(back);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            style={{
              ...
              alignItems: "center",
              gap: 6,
              color: ui.running ? "#00ee77" : "#ff5533",
              fontSize: 11,
            }}
          >
            <div
        ...
        <Acc
          label="◉ Estado línea"
          open={accordion.estado}
          onToggle={() => toggleAcc("estado")}
        >
          {SEGMENTS.map((seg) => {
              ...
              >
                <div
                  style={{
                    ...
                    background: sColor(s),
                    boxShadow:
                      s === "running" ? `0 0 5px ${sColor(s)}` : "none",
                    flexShrink: 0,
                  }}
                />
                <span style={{ color: "#6688aa", width: 32, flexShrink: 0 }}>
                  {seg.id}
                </span>
                <span
                  style={{
        ...
        <Acc
          label="⚡ Controles"
          open={accordion.controles}
          onToggle={() => toggleAcc("controles")}
        >
          {CTRL_DEFS.map((def) => {
                ...
                <div
                  style={{
                    ...
                    background: active
                      ...
                      : "#2a3444",
                    boxShadow: active
                      ? `0 0 6px ${isSeta ? "#ee1100" : "#ee6600"}`
                      : "none",
                    flexShrink: 0,
                  }}
                />
                <span style={{ color: "#6688aa", flex: 1 }}>{def.id}</span>
                <span style={{ color: active ? "#ffaa00" : "#3a4a5a" }}>
                  {active ? "ACTIVO" : "OK"}
                </span>
              </div>
            );
        ...
        >
          Three.js r128 · Gemelo Digital v4.0
        </div>
      </div>
